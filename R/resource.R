...
getResource <- function(resourceName = NULL, cache = FALSE) {
    if (!resourceName %in% c("GO-BP", "Reactome", "Network")) {
        message(cli::cli_alert_danger(".val {GO-BP, Reactome & Network} are the only keywords alllowed.", "\n"))
        stop()
    }
    ...
    if (cache == TRUE) {
        ...
        if (!dir.exists(resourcesCacheDir)) {
            message(cli::cli_alert_danger("Resources repository doesn't exist.", "\n"))
            stop()
        }
...
resetNetwork <- function(network) {
    ...
    message("\n")
    message(cli::cli_alert_info("New resource defined for {.val Network}.", "\n"))
    # message(utils::head(network))
...
resetPathwaysFromFile <- function(file,
    ...
    message("\n")
    message(cli::cli_alert_info("New resource defined for {.val {resourceName}}.", "\n"))
